...
import { Home, KeyRound, RefreshCw, ArrowLeft, ArrowLeftRight, FileSignature } from "lucide-react";
import {
  calculateBreakdown, DetailedBreakdown, formatGBP,
  LeaseholdType, TransactionType,
} from "@/lib/pricing";
...
const fieldClasses = "w-full rounded-lg border border-ink-900/15 bg-bone-50 px-4 py-3 text-sm text-ink-900 placeholder:text-ink-400 transition-colors focus:border-brass-500 focus:outline-none";
const slideVariants = { enter: { opacity: 0, x: 24 }, center: { opacity: 1, x: 0 }, exit: { opacity: 0, x: -24 } };

// ── Purchase-specific options section ────────────────────────────────────────

type PurchaseOptionsProps = {
  selectedOptions: string[];
  setSelectedOptions: (v: string[]) => void;
  giftedDepositCount: number; setGiftedDepositCount: (v: number) => void;
  htbIsaCount: number;        setHtbIsaCount: (v: number) => void;
  lifetimeIsaCount: number;   setLifetimeIsaCount: (v: number) => void;
};

function PurchaseOptions({
  selectedOptions, setSelectedOptions,
  giftedDepositCount, setGiftedDepositCount,
  htbIsaCount, setHtbIsaCount,
  lifetimeIsaCount, setLifetimeIsaCount,
}: Omit<PurchaseOptionsProps, "hasMortgage" | "includeSearchPack" | "setIncludeSearchPack">) {
    ...
    <div className="space-y-2">
      {PURCHASE_OPTIONS.map((opt) => (
        <div key={opt}>
          ...
          {opt === "Gifted Deposit" && selectedOptions.includes(opt) && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="ml-6 mt-2">
              <select value={giftedDepositCount} onChange={(e) => setGiftedDepositCount(Number(e.target.value))} className={fieldClasses}>
                <option value={0}>How many gifted deposits?</option>
                {[1,2,3,4,5].map((n) => <option key={n} value={n}>{n}</option>)}
              </select>
            </motion.div>
          )}

          {opt === "Help to Buy ISA" && selectedOptions.includes(opt) && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="ml-6 mt-2">
              <select value={htbIsaCount} onChange={(e) => setHtbIsaCount(Number(e.target.value))} className={fieldClasses}>
                <option value={0}>How many Help to Buy ISAs?</option>
                {[1,2].map((n) => <option key={n} value={n}>{n}</option>)}
              </select>
            </motion.div>
          )}

          {opt === "Lifetime ISA" && selectedOptions.includes(opt) && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="ml-6 mt-2">
              <select value={lifetimeIsaCount} onChange={(e) => setLifetimeIsaCount(Number(e.target.value))} className={fieldClasses}>
                <option value={0}>How many Lifetime ISAs?</option>
                {[1,2].map((n) => <option key={n} value={n}>{n}</option>)}
              </select>
            </motion.div>
          )}
...
// ── Generic checkbox group (sale / remortgage / transfer) ─────────────────────
...
// ── Generic property section (sale / remortgage / transfer) ───────────────────
...
function PropertySection(props: SectionProps) {
  const {
    ...
    showLeaseholdFloors, hasMortgage, setHasMortgage, showMortgage,
    showPeopleAddRemove, peopleBeingAdded, setPeopleBeingAdded,
    ...
    <div className="rounded-xl2 border border-ink-900/10 bg-bone-50 p-5">
      ...
      <div className="mt-4 space-y-4">
        <div>
          ...
          <AddressAutocomplete value={addressUnknown ? "" : address} onChange={setAddress} disabled={addressUnknown} required={!addressUnknown} className={fieldClasses} />
...
// ── Purchase section (with quantity sub-dropdowns) ────────────────────────────
...
type PurchaseSectionProps = {
  ...
  htbIsaCount: number;        setHtbIsaCount: (v: number) => void;
  lifetimeIsaCount: number;   setLifetimeIsaCount: (v: number) => void;
  includeSearchPack: boolean; setIncludeSearchPack: (v: boolean) => void;
};

function PurchaseSection(props: PurchaseSectionProps) {
  const {
    title, address, setAddress, addressUnknown, setAddressUnknown,
    propertyValue, setPropertyValue, peopleInvolved, setPeopleInvolved,
    isLeasehold, setIsLeasehold, leaseholdType, setLeaseholdType,
    ...
    htbIsaCount, setHtbIsaCount,
    lifetimeIsaCount, setLifetimeIsaCount,
    includeSearchPack, setIncludeSearchPack,
  } = props;
    ...
    <div className="rounded-xl2 border border-ink-900/10 bg-bone-50 p-5">
      ...
      <div className="mt-4 space-y-4">
        ...
        <div>
          ...
          />
          <label className="mt-2 flex cursor-pointer items-center gap-2 text-xs text-ink-500">
            <input type="checkbox" checked={addressUnknown}
              onChange={(e) => { setAddressUnknown(e.target.checked); if (e.target.checked) setAddress(""); }}
        ...
        <div>
          ...
          <input type="number" min={1} value={peopleInvolved}
            onChange={(e) => setPeopleInvolved(Number(e.target.value))}
            className={fieldClasses} required />
        </div>
        ...
        <div>
          ...
          <select value={hasMortgage ? "yes" : "no"}
        ...
        {/* Search pack — only shown when no mortgage */}
        {!hasMortgage && (
          <motion.div
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: "auto" }}
            exit={{ opacity: 0, height: 0 }}
            className="rounded-lg border border-brass-500/30 bg-brass-500/8 p-4"
          >
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-ink-700">
              Want the search pack included?
            </label>
            <p className="mb-2 text-xs text-ink-500">
              Includes Local Authority, Drainage & Water and Environmental Searches (£250)
            </p>
            <select
              value={includeSearchPack ? "yes" : "no"}
              onChange={(e) => setIncludeSearchPack(e.target.value === "yes")}
              className={fieldClasses}
            >
              <option value="yes">Yes — include search pack (£250)</option>
              <option value="no">No — I'll arrange searches separately</option>
            </select>
          </motion.div>
        )}
        ...
        <div>
          ...
          <div className="flex gap-5">
            {["Freehold","Leasehold"].map((opt) => (
              <label key={opt} className="flex cursor-pointer items-center gap-2 text-sm text-ink-700">
                <input type="radio" checked={isLeasehold === (opt === "Leasehold")}
          ...
          {isLeasehold && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-3">
              <select
                ...
                onChange={(e) => setLeaseholdType(e.target.value as LeaseholdType)}
                className={fieldClasses}
              >
                <option value="standard">The building has less than five floors</option>
                <option value="high-rise">The building has five or more floors</option>
        ...
        <div>
          ...
          <PurchaseOptions
            selectedOptions={additionalOptions} setSelectedOptions={setAdditionalOptions}
            giftedDepositCount={giftedDepositCount} setGiftedDepositCount={setGiftedDepositCount}
            htbIsaCount={htbIsaCount} setHtbIsaCount={setHtbIsaCount}
            lifetimeIsaCount={lifetimeIsaCount} setLifetimeIsaCount={setLifetimeIsaCount}
          />
        </div>
      </div>
    </div>
    ...
    <div className="overflow-hidden rounded-xl2 border border-ink-900/10 bg-bone-50">
      ...
      <div className="divide-y divide-ink-900/8">
        ...
        <div className="px-6 py-3">
          <p className="pb-1 text-xs font-semibold uppercase tracking-wide text-ink-500">Legal Fees</p>
          <div className="flex justify-between py-1.5 text-sm text-ink-700"><span>Legal Fees</span><span className="font-medium text-ink-900">{formatGBP(breakdown.legalFee)}</span></div>
          <div className="flex justify-between py-1.5 text-sm text-ink-700"><span>Legal Fees VAT at 20%</span><span className="font-medium text-ink-900">{formatGBP(breakdown.legalFeeVat)}</span></div>
        </div>
        ...
        {breakdown.supplements.length > 0 && (
          <div className="px-6 py-3">
            ...
            {breakdown.supplements.map((s) => (
              <div key={s.label} className="flex justify-between py-1.5 text-sm text-ink-700">
                <span>{s.label}</span><span className="font-medium text-ink-900">{formatGBP(s.amount)}</span>
              </div>
            ))}
            <div className="flex justify-between py-1.5 text-sm text-ink-700">
              <span>VAT at 20%</span><span className="font-medium text-ink-900">{formatGBP(breakdown.supplementsVat)}</span>
            </div>
          </div>
        )}
        ...
        {breakdown.disbursements.length > 0 && (
          <div className="px-6 py-3">
            ...
            {breakdown.disbursements.map((d) => (
              <div key={d.label} className="flex justify-between py-1.5 text-sm text-ink-700">
                <span>{d.label}</span><span className="font-medium text-ink-900">{formatGBP(d.amount)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {breakdown.sdltDeferred && (
          <div className="px-6 py-3">
            <div className="flex justify-between py-1.5 text-sm text-ink-700">
              <span>Stamp Duty (SDLT)</span><span className="font-medium text-ink-900">Deferred</span>
            </div>
            <p className="text-xs text-ink-400">Confirm exact figure with your solicitor before completion.</p>
          </div>
        )}
...
export default function InstantQuote() {
  ...
  const [purchaseIncludeSearchPack, setPurchaseIncludeSearchPack] = useState(true);
  const [giftedDepositCount, setGiftedDepositCount] = useState(0);
  const [htbIsaCount, setHtbIsaCount] = useState(0);
  const [lifetimeIsaCount, setLifetimeIsaCount] = useState(0);
...
function buildProceedUrl() {
    ...
    const addr =
      ...
      : type === "sale-purchase" ? saleAddress
    ...
    params.set("value", String(val));
    params.set("leasehold", String(lh));
    params.set("hasMortgage", String(hasMortgage));
    params.set("includeSearchPack", String(purchaseIncludeSearchPack));
    if (giftedDepositCount) params.set("giftedDepositCount", String(giftedDepositCount));
    if (htbIsaCount) params.set("htbIsaCount", String(htbIsaCount));
    if (lifetimeIsaCount) params.set("lifetimeIsaCount", String(lifetimeIsaCount));

    const opts = [...saleOptions, ...purchaseOptions, ...remortgageOptions, ...toeOptions];
  ...
  function handleReset() {
    ...
    setSaleAddress(""); setSaleAddressUnknown(false); setSaleValue(0); setSalePeople(1); setSaleLeasehold(false); setSaleLeaseholdType("standard"); setSaleOptions([]);
    setPurchaseAddress(""); setPurchaseAddressUnknown(false); setPurchaseValue(0); setPurchasePeople(1); setPurchaseLeasehold(false); setPurchaseLeaseholdType("standard"); setPurchaseOptions([]); setHasMortgage(true); setPurchaseIncludeSearchPack(true); setGiftedDepositCount(0); setHtbIsaCount(0); setLifetimeIsaCount(0);
    setRemortgageAddress(""); setRemortgageAddressUnknown(false); setRemortgagePropValue(0); setRemortgagePeople(1); setRemortgageLeasehold(false); setRemortgageOptions([]);
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const purchaseSectionData = (type === "purchase" || type === "sale-purchase") ? {
      ...
      includeSearchPack: purchaseIncludeSearchPack,
    } : null;
    ...
    if (type === "sale-purchase") {
      ...
      purchaseBreakdown = calculateBreakdown("purchase", {
        ...
        hasMortgage, includeSearchPack: purchaseIncludeSearchPack,
        selectedOptions: purchaseOptions, peopleInvolved: purchasePeople,
        giftedDepositCount, htbIsaCount, lifetimeIsaCount,
        forceNotFirstTimeBuyer: true,
      });
    ...
    const payload = {
      ...
      message: formData.get("message") || "", honeypot: formData.get("website"),
      ...
      propertyValue: type === "remortgage" ? remortgagePropValue : type === "transfer-of-equity" ? toePropValue : null,
      isLeasehold: type === "remortgage" ? remortgageLeasehold : type === "transfer-of-equity" ? toeLeasehold : false,
      leaseholdType: null,
      peopleInvolved: type === "remortgage" ? remortgagePeople : type === "transfer-of-equity" ? toePeople : 1,
      hasMortgage: (type === "purchase" || type === "sale-purchase") ? hasMortgage : null,
      includeSearchPack: (type === "purchase" || type === "sale-purchase") ? purchaseIncludeSearchPack : null,
      giftedDepositCount: (type === "purchase" || type === "sale-purchase") ? giftedDepositCount : null,
      htbIsaCount:        (type === "purchase" || type === "sale-purchase") ? htbIsaCount : null,
      lifetimeIsaCount:   (type === "purchase" || type === "sale-purchase") ? lifetimeIsaCount : null,
      remortgageValue: type === "remortgage" ? remortgagePropValue : null,
      ...
      additionalOptions: type === "remortgage" ? remortgageOptions : type === "transfer-of-equity" ? toeOptions : [],
      saleSection: saleSectionData, purchaseSection: purchaseSectionData,
      saleBreakdown, purchaseBreakdown, singleBreakdown, combinedTotal,
    };
            ...
            <form onSubmit={(e) => { e.preventDefault(); setStep(3); }}>
              ...
              <div className="mt-5 space-y-5">
                ...
                {(type === "purchase" || type === "sale-purchase") && (
                  <PurchaseSection title="Buying a property"
                    ...
                    hasMortgage={hasMortgage} setHasMortgage={setHasMortgage}
                    giftedDepositCount={giftedDepositCount} setGiftedDepositCount={setGiftedDepositCount}
                    htbIsaCount={htbIsaCount} setHtbIsaCount={setHtbIsaCount}
                    lifetimeIsaCount={lifetimeIsaCount} setLifetimeIsaCount={setLifetimeIsaCount}
                    includeSearchPack={purchaseIncludeSearchPack} setIncludeSearchPack={setPurchaseIncludeSearchPack} />
                )}
                {type === "remortgage" && (
...
/>
              ...
              <p className="text-xs leading-relaxed text-ink-500">By submitting, you agree to our Privacy Policy. We'll only use your details to send your quote.</p>
        ...
        {step === 4 && (
          <motion.div key="step4" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }}>
            ...
            <p className="mt-1 text-sm text-ink-600">Full breakdown — nothing hidden.</p>
            ...
            <div className="mt-5 space-y-5">
              ...
              {resultSingleBreakdown   && (
                <BreakdownPanel
                  title={type === "sale" ? "Sale" : type === "purchase" ? "Purchase" : type === "remortgage" ? "Remortgage" : "Transfer of Equity"}
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <a href="/discuss-quote"
                onClick={async () => {
                  if (leadId) await fetch(`/api/leads/${leadId}/intent`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ intent: "discuss" }) }).catch(() => {});
                }}
