...
type SectionProps = {
  ...
  remortgageValue?: number;
  setRemortgageValue?: (v: number) => void;
      ...
      <div className="mt-4 space-y-4">
        ...
        {showRemortgageValue && (
          <div>
            ...
            <CurrencyInput
              value={remortgageValue ?? 0}
              onChange={(v) => setRemortgageValue?.(v)}
              className={fieldClasses}
              required
            />
          </div>
    ...
    <div className="overflow-hidden rounded-xl2 border border-ink-900/10 bg-bone-50">
      ...
      <div className="divide-y divide-ink-900/8">
        <div className="px-6 py-2">
          <p className="pt-2 text-xs font-medium uppercase tracking-wide text-ink-500">
            Legal Fees
          </p>
        ...
        {breakdown.supplements.length > 0 && (
          <div className="px-6 py-2">
            ...
            {breakdown.supplements.map((s) => (
              <div
                key={s.label}
                className="flex justify-between py-2 text-sm text-ink-700"
              >
                <span>{s.label}</span>
                <span className="font-medium text-ink-900">
                  {formatGBP(s.amount)}
        ...
        <div className="flex justify-between bg-ink-900/[0.03] px-6 py-4">
          <span className="font-display text-base font-semibold text-ink-900">
            Total, including VAT and disbursements
          </span>
      ...
      {breakdown.unpricedOptions.length > 0 && (
        <p className="border-t border-ink-900/10 px-6 py-3 text-xs text-ink-500">
          Also selected: {breakdown.unpricedOptions.join(", ")} — these may
          carry additional fees, confirmed when your solicitor reviews the file.
        </p>
      )}
    </div>
  );
...
export default function InstantQuote() {
  ...
  const [resultSaleBreakdown, setResultSaleBreakdown] =
    useState<DetailedBreakdown | null>(null);
  const [resultPurchaseBreakdown, setResultPurchaseBreakdown] =
    useState<DetailedBreakdown | null>(null);
  const [resultSingleBreakdown, setResultSingleBreakdown] =
    useState<DetailedBreakdown | null>(null);
  const [resultCombinedTotal, setResultCombinedTotal] = useState<number | null>(
    null
  );
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (type === "sale-purchase") {
      ...
      purchaseBreakdown = calculateBreakdown("purchase", {
        ...
        leaseholdType: purchaseLeasehold ? purchaseLeaseholdType : null,
        hasMortgage,
        selectedOptions: purchaseOptions,
  ...
  async function handleProceed() {
    if (!leadId) return;
    setProceedStatus("loading");
    try {
      await fetch(`/api/leads/${leadId}/intent`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ intent: "proceed" }),
      });
      setProceedStatus("done");
    } catch {
      setProceedStatus("idle");
    }
  }
  ...
  function handleReset() {
    ...
    setType(null);
    setSaleAddress("");
    setSaleAddressUnknown(false);
    setSaleValue(0);
    setSalePeople(1);
    setSaleLeasehold(false);
    setSaleLeaseholdType("standard");
    setSaleOptions([]);
    setPurchaseAddress("");
    setPurchaseAddressUnknown(false);
    setPurchaseValue(0);
    setPurchasePeople(1);
    setPurchaseLeasehold(false);
    setPurchaseLeaseholdType("standard");
    setPurchaseOptions([]);
    setHasMortgage(true);
    setRemortgageAddress("");
    setRemortgageAddressUnknown(false);
    setRemortgagePropValue(0);
    setRemortgageValue(0);
    setRemortgagePeople(1);
    setRemortgageLeasehold(false);
    setRemortgageOptions([]);
    setToeAddress("");
    setToeAddressUnknown(false);
    setToePropValue(0);
    setToePeopleAdded(0);
    setToePeopleRemoved(0);
    setToeLeasehold(false);
    setToeOptions([]);
    setSubmitError(null);
    setLeadId(null);
    setResultSaleBreakdown(null);
    setResultPurchaseBreakdown(null);
    setResultSingleBreakdown(null);
    setResultCombinedTotal(null);
    setProceedStatus("idle");
  }
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 3 && (
          ...
          >
            ...
            <p className="mt-1 text-sm text-ink-600">
              We need these before we can show you your quote — a solicitor will
              confirm it by email, usually within one working day.
            </p>
        ...
        {step === 4 && (
          ...
          >
            ...
            <p className="mt-1 text-sm text-ink-600">
              The full breakdown — nothing hidden, nothing combined that
              shouldn't be.
            </p>
            ...
            {resultCombinedTotal !== null && (
              <div className="mt-5 rounded-xl2 bg-ink-900 p-6 text-bone-100">
                <p className="text-xs uppercase tracking-wide text-bone-100/60">
                  Combined total (sale + purchase)
                </p>
                <p className="mt-1 font-display text-4xl">
                  {formatGBP(resultCombinedTotal)}
                </p>
              </div>
            )}
            ...
            <div className="mt-5 space-y-5">
              {resultSaleBreakdown && (
                <BreakdownPanel title="Sale" breakdown={resultSaleBreakdown} />
              )}
              {resultPurchaseBreakdown && (
                <BreakdownPanel
                  title="Purchase"
                  breakdown={resultPurchaseBreakdown}
                />
              )}
            ...
            <p className="mt-4 text-xs text-ink-500">
              Estimate only. SDLT is calculated using standard England rates and
              doesn't account for every relief or surcharge — your solicitor
              will confirm the exact figure. Final costs may vary based on title
              complexity and search results.
            </p>

            {proceedStatus === "done" ? (
              <div className="mt-7 rounded-xl2 border border-brass-500/30 bg-brass-500/10 p-5 text-sm text-ink-800">
                Thanks — we've noted you'd like to proceed. A solicitor will be
                in touch shortly to formally open your file.
              </div>
            ) : (
              <div className="mt-7 flex flex-wrap gap-3">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  type="button"
                  disabled={proceedStatus === "loading"}
                  onClick={handleProceed}
                  className="rounded-full bg-brass-500 px-6 py-3.5 text-sm font-medium text-ink-950 shadow-soft hover:shadow-lift disabled:opacity-60"
                >
                  {proceedStatus === "loading"
                    ? "Sending..."
                    : "Proceed with Quote"}
                </motion.button>
                <button
